...
export class Element {
  ...
  /** The element as an action's `origin`: an input action placed relative to its centre. */
  reference(): Record<string, string> {
    return { [ELEMENT_KEY]: this.id };
  }
...
export const Keys = {
  // Written as escapes, so the code point is readable: F3 and Shift+F3 step through the search
  // hits, F4 shows and hides the side panel, and F6 moves between the Viewer's regions
  // (F1 experience §3.10).
  F3: '',
  F4: '',
  F6: '',
...
export class Driver {
  ...
  /**
   * Perform W3C input sources together — keys, a pointer, a wheel — tick by tick, then release
   * everything. What a person does with both hands, such as Ctrl held under a wheel notch.
   */
  async perform(sources: unknown[]): Promise<void> {
    await call('POST', this.sessionUrl('/actions'), { actions: sources });
    await call('DELETE', this.sessionUrl('/actions'));
  }

  /** Resize the window (outer size, in CSS pixels). */
  async setWindowRect(width: number, height: number): Promise<void> {
    await call('POST', this.sessionUrl('/window/rect'), { width, height });
  }
  ...
  /**
   * Press one key `times` times, as a person does — down, then up — in one action sequence, to the
   * element that has focus.
   */
  async press(key: string, times = 1): Promise<void> {
    const actions = Array.from({ length: times }, () => [
  ...
  /** The text of an open user prompt — an `alert()` — or a `no such alert` error when there is none. */
  alertText(): Promise<string> {
    return call('GET', this.sessionUrl('/alert/text'));
  }
