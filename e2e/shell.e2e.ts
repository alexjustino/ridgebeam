...
describe('shell', () => {
  ...
  it('lists every destination, in order, and disables the two that need a work', async () => {
    const { driver } = session;
    ...
    expect(ids).toEqual(['dashboard', 'plan', 'settings', 'diagnostics', 'about']);
    const disabled = await Promise.all(buttons.map((b) => b.attribute('aria-disabled')));
    expect(disabled).toEqual(['true', 'true', null, null, null]);
  });
