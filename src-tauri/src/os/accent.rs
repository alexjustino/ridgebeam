...
/// The accent ramp, as CSS hex strings. Serialised in camelCase, like every
/// shape on the boundary (`fromSystem`).
...
mod tests {
    ...
    #[test]
    fn the_ramp_reaches_the_interface_in_camel_case() {
        let value = serde_json::to_value(AccentRamp::default()).unwrap();
        assert_eq!(value["fromSystem"], false);
        assert!(value.get("from_system").is_none());
    }
