 ...
 *
 * Two claims the specification makes about a release (SPEC §4, §5):
 *
 *   1. The installer is under 10 MB. A local-first desktop application that
 *      arrives as a hundred megabytes has stopped being one, and a planner
 *      whose rules are TypeScript and whose store is SQLite has no reason to
 *      weigh more than a few.
...
function installers() {
  ...
  for (const [dir, extension] of [
    ...
    for (const entry of entries) {
      // `Ridgebeam_1.0.0_x64_en-US.msi` and `Ridgebeam_1.0.0_x64-setup.exe`: the
...
for (const file of built) {
  ...
  if (bytes > MAX_BYTES) {
    problems.push(`${name} is ${megabytes} MB — the ceiling is 10 MB`);
  } else {
...
// that happens to sit in the release folder. The needle is the checkout's own
// path, `src\ridgebeam\src-tauri` with single backslashes: a debug build carries
// it and a stripped release build does not. (The pattern this was copied from
// asked for doubled backslashes, matched neither build, and so passed anything;
// both patterns were tried against a sibling's debug and release binaries
// before this one was chosen.)
const exe = path.join(root, 'src-tauri', 'target', 'release', 'ridgebeam.exe');
try {
  const bytes = statSync(exe).size;
  console.log(`  ridgebeam.exe  ${(bytes / (1024 * 1024)).toFixed(2)} MB`);
  const symbols = execFileSync(
    ...
    [
      ...
      `$bytes = [System.IO.File]::ReadAllBytes('${exe.replace(/'/g, "''")}'); ` +
        ...
        `if ($text -match 'src\\\\ridgebeam\\\\src-tauri') { 'yes' } else { 'no' }`,
    ],
    { encoding: 'utf-8' },
  ).trim();
  if (symbols === 'yes') {
    problems.push('ridgebeam.exe carries build paths — it was not stripped');
  }
} catch (error) {
  problems.push(`ridgebeam.exe could not be inspected: ${error.message}`);
}
